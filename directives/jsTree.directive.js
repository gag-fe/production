...
ngJSTree.directive('jsTree', ['$http', function($http) {
  ...
  var treeDir = {
    ...
    managePlugins: function(s, e, a, config) {
      if (a.treePlugins) {
        ...
        if (config.plugins.indexOf('search') >= 0) {
          var to = false;
          if (!e.next().hasClass('ng-tree-search')) {
            e.after('<input type="text" placeholder="请输入机构名称检索" class="ng-tree-search form-control"/>')
              .next()
    ...
    // users can define 'core'
        config.core = {expand_selected_onload: false};
        if (a.treeCore) {
